...
export function WeeklySummaryModal({ open, onClose, weekLabel, weekStart }: { open: boolean; onClose: () => void; weekLabel: string; weekStart: string }) {
  ...
  useEffect(() => {
    if (editIdx !== null) draftRef.current?.select();
  }, [editIdx]);
  ...
  const addPoint = () => { setDraft(''); setEditIdx(bullets.length); setBullets((b) => [...b, '']); };
  ...
  const runGenerate = async () => {
    try {
      const res = await generate.mutateAsync(weekStart);
      setBullets(res.bullets);
      toast('Summary generated', 'success');
    } catch (e) { toast(apiErrorMessage(e, 'Generation failed'), 'error'); }
  };

  // hasBullets = at least one NON-empty bullet (drives "found" styling / Regenerate
  // label); bullets.length alone drives which block renders below, so a freshly
  // added-but-still-empty point (via "Add a point") stays visible instead of being
  // hidden behind the "No summary yet" empty state.
      ...
      <div className="modal modal-lg">
        ...
        <div className="modal-bd">
          <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>
            {weekLabel}
            <span style={{ background: 'var(--p3)', color: 'var(--p)', borderRadius: 10, padding: '1px 8px', fontSize: 10, marginLeft: 6 }}>editable</span>
          </div>
                ...
                {bullets.map((b, i) => (
                  editIdx === i ? (
                    <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                      <textarea ref={draftRef} className="fc" rows={2} autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} style={{ flex: 1 }} />
                    ...
                    b.trim() ? (
                      <div key={i} onClick={() => startEdit(i)} className="ws-bullet-row" style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '6px 4px', borderRadius: 6, cursor: 'pointer' }}>
        ...
        <div className="modal-ft" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <button className="btn btn-primary btn-sm" disabled={generate.isPending} onClick={runGenerate}>
              <Icon name="auto_awesome" size={15} /> {hasBullets ? 'Regenerate' : 'Generate Now'}
            </button>
            <button className="btn btn-ghost btn-sm" onClick={copyAll}><Icon name="content_copy" size={15} /> Copy</button>
            {savedHint && (
