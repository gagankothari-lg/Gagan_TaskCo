...
export function ProjectDetailModal({ projId, onClose }: { projId: string | null; onClose: () => void }) {
  ...
  const canEdit = project ? canEditProject(currentUser, project) : false;
  const canDelete = project ? canDeleteProject(currentUser, project) : false;
  ...
  async function remove() {
    if (!confirm('Delete this project? Its tasks, functions and sub-functions will also be removed.')) return;
    try {
  ...
  return (
    <Dialog open={!!projId} onOpenChange={(next) => { if (!next) onClose(); }}>
      <DialogContent size="lg">
        <DialogHeader>
          <div className="min-w-0 flex-1">
            <DialogTitle className="truncate">{project?.name ?? 'Project'}</DialogTitle>
            <p className="mt-0.5 text-xs text-muted">Project · {project?.projId ?? projId}</p>
          </div>
        </DialogHeader>
...
function Row({ id, label }: { id: string; label: string }) {
  return (
    <div className="flex items-center gap-2 rounded-[8px] border border-border bg-bg px-3 py-1.5">
