...
export function CreateFunctionModal({ open, onClose, defaultProjId, defaultParentFnId }: CreateFunctionModalProps) {
  ...
  }, [open, defaultProjId, defaultParentFnId]);
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="space-y-3 px-5 py-4">
                    ...
                    <FormItem>
                      <FormControl>
                        <select className={fieldClass} {...field}>
                          ...
                          {(functions ?? []).filter((f) => !f.parentFnId).map((f) => <option key={f.functionId} value={f.functionId}>{f.functionId} — {f.name}</option>)}
                        </select>
                      </FormControl>
                      <FormMessage />
                    </FormItem>
              ...
              <div>
                <label className="mb-1 block text-xs text-muted">Assignees</label>
                {manager ? (
                  <FormField
                    control={form.control}
                    name="assigneeIds"
                  ...
                  <div className="flex items-center gap-2 rounded-[8px] border border-border bg-bg px-3 py-2 text-sm text-muted">
                    <Icon name="info" size={14} className="text-warn" /> Assigned to you (members can only self-assign functions)
                  </div>
                )}
              </div>
