...
export function WorkRow({ date, entry, isIntern, isManager, locked, durationHms, pickerItems, onSave }: WorkRowProps) {
  ...
  const [internText, setInternText] = useState(entry?.attendance ?? '');
  const [h1, setH1] = useState<Chip[]>(() => parseChips(entry?.work1stHalf));
  const [h2, setH2] = useState<Chip[]>(() => parseChips(entry?.work2ndHalf));
  const [extraHours, setExtraHours] = useState<number>(entry?.extraHours ?? 0);
  ...
  const showsOt = ALWAYS_OT.includes(attendance) || offDayWorked || extraHours > 0;
  ...
  async function doSave() {
    ...
    const att = isIntern ? internText : attendance;
    const w1 = serializeChips(h1);
    const w2 = serializeChips(h2);
    // Empty-row short-circuit.
    if (!att && !w1 && !w2 && !remark && !extraHours) { dirty.current = false; setSaveState('idle'); return; }
    saving.current = true;
  ...
  return (
    <tr className={rowCls}>
      ...
      <td className="wl-att-cell" style={{ minWidth: 150 }}>
        {isIntern ? (
          <input
            className="wl-inp" value={internText} disabled={locked}
            placeholder="e.g. 9, 8.5, Holiday, Leave"
            onChange={(e) => setInternText(e.target.value)}
          />
        ) : (
          <>
            <select
              className="wl-inp" value={attendance} disabled={locked}
      ...
      <td className="wl-extra-cell">
        <input type="number" min={0} max={12} step={0.5} className="wl-inp" style={{ width: 60 }} disabled={locked}
          value={extraHours} onChange={(e) => setExtraHours(Math.max(0, parseFloat(e.target.value) || 0))} />
      </td>

      {/* Remark */}
      <td className="wl-remark-cell">
        <input className="wl-inp" placeholder="Remark…" disabled={locked} value={remark} onChange={(e) => setRemark(e.target.value)} />
      </td>

      {/* Status */}
      <td className="wl-status-cell">
        {isManager ? (
          <select className="wl-inp" value={status} onChange={(e) => setStatus(e.target.value)}>
      ...
      <td className="wl-comments-cell">
        {isManager ? (
          <input className="wl-inp" placeholder="Comment…" value={comments} onChange={(e) => setComments(e.target.value)} />
        ) : (
