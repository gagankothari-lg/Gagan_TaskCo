...
interface WeekRowProps {
  date: Date;
  entry?: WorkLogEntry;
  save: (input: WorkLogInput) => Promise<void>;
  showStatusComment?: boolean;
  durationHms?: string;
}

export function WeekRow({ date, entry, save, showStatusComment, durationHms }: WeekRowProps) {
  ...
  return (
    <tr className="border-b border-[var(--border)] bg-[var(--surface)] align-top">
      <td className="px-3 py-2.5 whitespace-nowrap">
        <span className={['text-sm', isToday ? 'font-semibold text-[var(--p-fg)]' : 'text-[var(--text)]'].join(' ')}>
          {date.toLocaleDateString(undefined, { weekday: 'short' })}
        </span>
        <span className="ml-1 text-xs text-[var(--muted)]">{date.getDate()}</span>
      </td>
      <td className="px-3 py-2.5">
        <select className={fieldClass} value={form.attendance} onChange={(e) => update({ attendance: e.target.value })}>
      ...
      {showStatusComment && (
        <td className="px-3 py-2.5">
          <input className={`${fieldClass} mb-1 w-full`} placeholder="Status" value={form.status ?? ''} onChange={(e) => update({ status: e.target.value })} />
          <input className={`${fieldClass} w-full`} placeholder="Comment" value={form.comments ?? ''} onChange={(e) => update({ comments: e.target.value })} />
        </td>
      )}
